...
public class SfTeam implements NwITeam {
    ...
    public Map<NwITeam, TeamIRelation> getAllies() {
        ...
        factions.getAllFactions().stream().filter(otherFaction -> (
            ...
            SfTeam allySfTeam = (SfTeam) teamManager.getStringTeamMap().values().stream().filter(nwITeam ->
                    ((SfTeam) nwITeam).faction == otherFaction).findFirst().orElse(null);

            if (allySfTeam != null) {
                NwTeamRelation allyRelation = new NwTeamRelation(this, allySfTeam, RelationType.ALLY, null);
                allies.put(allySfTeam, allyRelation);
            }
        });
    ...
    public Map<NwITeam, TeamIRelation> getTruce() {
        ...
        factions.getAllFactions().stream().filter(otherFaction -> (
            ...
            SfTeam truceSfTeam = (SfTeam) teamManager.getStringTeamMap().values().stream().filter(nwITeam ->
                    ((SfTeam) nwITeam).faction == otherFaction).findFirst().orElse(null);

            if (truceSfTeam != null) {
                NwTeamRelation truceRelation = new NwTeamRelation(this, truceSfTeam, RelationType.TRUCE, null);
                truces.put(truceSfTeam, truceRelation);
            }
        });
    ...
    public Map<NwITeam, TeamIRelation> getEnemies() {
        ...
        factions.getAllFactions().stream().filter(otherFaction -> (
            ...
            SfTeam rivalSfTeam = (SfTeam) teamManager.getStringTeamMap().values().stream().filter(nwITeam ->
                    ((SfTeam) nwITeam).faction == otherFaction).findFirst().orElse(null);

            if (rivalSfTeam != null) {
                NwTeamRelation enemyRelation = new NwTeamRelation(this, rivalSfTeam, RelationType.ENEMY, null);
                enemies.put(rivalSfTeam, enemyRelation);
            }
        });
