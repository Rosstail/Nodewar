...
public class UltimateClanTeam implements NwITeam {
    ...
    public Map<NwITeam, TeamIRelation> getAllies() {
        ...
        clanData.getRivalAlly().getAlly().forEach(allyUuid -> {
            UltimateClanTeam allyUltimateClanTeam = (UltimateClanTeam) teamManager.getStringTeamMap().values().stream().filter(nwITeam ->
                    ((UltimateClanTeam) nwITeam).clanData.getId().equals(allyUuid)
            ).findFirst().orElse(null);
            if (allyUltimateClanTeam != null) {
                NwTeamRelation allyRelation = new NwTeamRelation(this, allyUltimateClanTeam, RelationType.ALLY, null);
                allies.put(allyUltimateClanTeam, allyRelation);
            }
        });
    ...
    public Map<NwITeam, TeamIRelation> getEnemies() {
        ...
        clanData.getRivalAlly().getRival().forEach(rivalUuid -> {
            UltimateClanTeam rivalUltimateClanTeam = (UltimateClanTeam) teamManager.getStringTeamMap().values().stream().filter(nwITeam ->
                    ((UltimateClanTeam) nwITeam).clanData.getId().equals(rivalUuid)
            ).findFirst().orElse(null);

            if (rivalUltimateClanTeam != null) {
                NwTeamRelation enemyRelation = new NwTeamRelation(this, rivalUltimateClanTeam, RelationType.ENEMY, null);
                enemies.put(rivalUltimateClanTeam, enemyRelation);
            }
        });
